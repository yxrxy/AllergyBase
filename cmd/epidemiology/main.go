...
import (
	...
	"github.com/yxrxy/AllergyBase/app/epidemiology"
...
func init() {
	config.Init("epidemiology")
}

func main() {
	r, err := etcd.NewEtcdRegistry([]string{config.Etcd.Addr})
	if err != nil {
		log.Fatalf("Epidemiology: new etcd registry failed, err: %v", err)
	}
	listenAddr := config.Epidemiology.RPCAddr
	addr, err := net.ResolveTCPAddr("tcp", listenAddr)
	if err != nil {
		log.Fatalf("Epidemiology: resolve tcp addr failed, err: %v", err)
	}
	p := base.TelemetryProvider(config.Server.Name, config.Otel.CollectorAddr)
	defer func() {
		if err := p.Shutdown(context.Background()); err != nil {
			log.Fatalf("Epidemiology: shutdown telemetry provider failed, err: %v", err)
		}
	}()

	svr := epidemiologyservice.NewServer(
		// 注入依赖
		epidemiology.InjectEpidemiologyHandler(),
	...
	if err = svr.Run(); err != nil {
		log.Printf("Epidemiology: run server failed, err: %v", err)
		return
	}
}
